...
`batch-validate` 现已按同样口径输出：

- 7 个确定性策略各自的数量、占比、验证通过率、补丁准确率
- `L0-L5` 每一级的单独准确率
- 自动化覆盖率 vs 仍需纯人工比例
...
# Q10: 关联补丁与 DryRun 依赖分析方法（为什么能节省人工）

### 质疑

> 工具虽然给出结论，但人工仍然要重新分析前置补丁，工作量并没有减少。

### 回应（拟人化分析过程 + 可审计证据链）

```
先明确角色分工:
  工具不是替代人，而是替人完成“80%机械分析”，
  人工负责“20%最终裁决”。

我们把“前置补丁分析”拆成 5 步，每一步都留证据:

Step 1: 建候选集（2-5 分钟，工具自动）
  过去人工做法:
    - 在 git log 里按关键词翻几十到几百个 commit
    - 猜哪些可能是前置
  工具做法:
    - 只抓漏洞引入后、修复前的时间窗口
    - 只保留同文件/同函数/同 hunk 重叠的 commit
    - 输出 strong/medium/weak 分级候选
  人工动作:
    - 只看 strong + 少量 medium

Step 2: 做“空集基线”DryRun（30 秒内，工具自动）
  关键问题: 不带任何前置补丁，修复补丁能不能直接落地？
  证据:
    - strict/fuzz/3way/verified-direct 的尝试轨迹
    - 每个失败 hunk 的 reject 位置与原因

Step 3: 做“最小集合”隔离实验（1-3 分钟，工具主导）
  关键问题: 到底是哪个前置补丁“真正必须”？
  方法:
    - 单补丁试验: {A}、{B}、{C}
    - 组合试验: {A,B}、{A,C} ...
...
Step 4: 语义兜底（人工 3-8 分钟，工具给检查清单）
  避免“能 apply 但语义错位”:
    - 是否引入新 struct 字段/API/宏语义变化
    - 是否仅是命名差异（可由符号映射解释）
    - 关键路径是否通过编译/单测/最小回归

Step 5: 产出可答辩结论（工具自动生成 narrative）
  结论模板:
    - 必须前置（Hard）/建议前置（Soft）/无前置（No prerequisite）
    - 置信度: 高/中/低
    - 边界: “仅对当前分支 + 当前配置成立”

为什么这叫“节省工作量”？
  节省的不是“思考”，而是“检索 + 枚举 + 复现实验”:
    - 人工全做: 60~120 分钟
    - 工具预分析 + 人工裁决: 8~15 分钟
  人工从“重跑流程”变成“审阅证据”。
```
...
| 维度   | 关联补丁分析       | DryRun          |
| ---- | ------------ | --------------- |
| 作用   | 给出“可能依赖谁”    | 验证“是否真的依赖”      |
| 输出   | 候选集 + 依赖分级   | 可应用性轨迹 + 失败证据   |
| 风险   | 仅看关联会误报      | 仅看 apply 会漏语义依赖 |
| 正确姿势 | 两者联用，做最小集合验证 |                 |


**一句话：关联补丁分析负责“提名候选人”，DryRun 负责“法庭举证”，人工负责“最终判决”。**
...
| **L2** | **Controlled Review**             | 补丁能打上，但已经不是“原样回移”，需要确认自动合并后的结果仍然正确         | 常见于 `3way`：社区补丁和目标仓都改了同一段代码，Git 借助共同祖先把两边改动自动合并；或者出现大改动、返回路径变化、错误路径、中等依赖、API surface 变化 | 逐 hunk 对照主线，确认“两边改动叠加后”的语义是否仍成立 | **人工核对为主**      |
...
# 调用链 fanout / 传播链：这里具体指什么？

这一块最容易被用户误解，所以要单独讲清楚：

- 这里的“调用链”不是指“全仓库无限递归地向外追踪很多层”
- 当前实现看的是：**本次补丁修改到的函数，在本次分析覆盖到的修改文件集合里，有多少直接 caller 和 direct callee**
- 换句话说，它更接近“局部调用牵连图”，不是无限展开的全局架构图

### 先把 4 个词讲清楚

| 术语 | 在当前实现里具体是什么意思 | 用户应该怎么理解 |
| --- | --- | --- |
| `caller` | 哪些函数会调用当前被修改函数 | 上游谁依赖它 |
| `callee` | 当前被修改函数又调用了哪些函数 | 下游它会影响谁 |
| `fanout` | `callers + callees` 的数量总和 | 这个函数和周边函数耦合得有多广 |
| `propagation` | 修改风险不只停留在当前 hunk，而是顺着 caller/callee 关系扩散到更多函数 | 风险已经从“单点改动”变成“链路改动” |

### fanout 阈值到底是什么？

| 项目 | 当前含义 | 默认值 |
| --- | --- | --- |
| `call_chain_fanout_threshold` | 当某个被修改函数的 `callers + callees` 达到这个数，就认为“调用链扩散较大” | 平衡风格 `6`，保守风格 `4` |
| `call_chain_promotion_min_fanout` | 用于 `call_chain_propagation` 的最小触发门槛；如果没有命中关键结构，至少要达到这个 fanout 才会因为传播链升级 | 当前默认 `2` |

可以把它理解成：

- `fanout_threshold` 解决的是“这条链路是不是已经明显变宽了”
- `promotion_min_fanout` 解决的是“即使还没很宽，是否也已经足够说明风险开始传播了”

### `call_chain_fanout` 和 `call_chain_propagation` 有什么区别？

| 规则 | 触发方式 | 常见 floor | 用户理解 |
| --- | --- | --- | --- |
| `call_chain_fanout` | 修改函数的 `fanout >= threshold` | `L2` | 说明这不是孤立函数，调用影响面已经比较大 |
| `call_chain_propagation` | 修改函数存在 caller/callee 牵连；若无关键结构，通常要 `fanout >= 2`；若叠加关键结构，则哪怕只有少量牵连也会触发更高升级 | 无关键结构时常见到 `L2`；叠加关键结构时会抬到 `L4` | 说明风险已经不是“局部看一眼 hunk”能结束，而要顺着链路看上下游影响 |

最关键的一句：

- `call_chain_fanout` 更像“影响面变大”
- `call_chain_propagation` 更像“风险开始沿链路扩散”

### 为什么调用链会导致 L0/L1 升到更高等级？

因为低级别通道默认假设是：

- 修改点局部、边界清楚
- 补丁影响主要停留在当前 hunk / 当前函数
- 审查者不需要沿着一串调用关系继续追上下游

一旦出现明显 caller/callee 牵连，情况就变了：

- 你不再只审“这几行改得对不对”
- 还要审“调用者的前提还成立吗”
- 还要审“被调用者返回值、状态、锁域、字段语义是否仍匹配”

所以调用链风险通常至少把样本抬到 `L2`；如果它和锁、生命周期、状态机、字段路径等关键结构叠加，就很容易继续抬到 `L4`。
...
- 只改当前 hunk 本身，叫“局部 patch 风险”
- 但如果 `fix_A()` 的调用前提、`check_state()` 的状态判断、`free_ctx()` 的资源释放都可能被连带影响
- 那这就不是“改了一个函数”，而是“改动开始沿调用链传播”

### 对外汇报时，建议直接这样说

| 用户看到的现象 | 建议解释口径 |
| --- | --- |
| `fanout` 高 | 这个补丁改到的函数和周边函数连接很多，影响面不再局限于当前 hunk |
| 命中 `call_chain_fanout` | 说明需要把它当“多函数联动”问题，而不是单点文本适配问题 |
| 命中 `call_chain_propagation` | 说明风险已经沿调用者 / 被调用者链路扩散，必须看上下游语义是否仍成立 |
| `关键结构 + propagation` 把样本抬到 `L4` | 不是因为代码行数多，而是因为并发/生命周期/字段等高风险语义已经扩散到了链路级 |

---

# 用户理解版：常见基线方法到底意味着什么？

| 基线方法 | 工具内部发生了什么 | 用户应该怎么理解 | 为什么不能再算低级别 |
| ------ | ---------- | --------- | ---------- |
| `strict` | 原始补丁直接 `git apply --check` 通过 | 目标仓代码和社区补丁上下文基本一致 | 只有在没有额外风险信号时，才真的留在 `L0` |
| `context-C1 / ignore-ws` | 放宽上下文或空白匹配后通过 | 补丁主体没变，但周边代码或格式已有轻微漂移 | 说明已经不是“完全原样”，所以通常至少要做轻量复核 |
| `3way` | Git 通过共同祖先，把“社区修改”和“目标仓已有修改”自动合并成一个结果 | 补丁能打上，但打上的结果是“两边改动叠加后的产物”，不是社区补丁原样落地 | 风险不在“能不能打上”，而在“叠加后的语义是否仍然正确”，所以进入 `L2` |
| `verified-direct` | 工具绕过 `git apply`，在内存里定位、验证并重建 diff | 代码意图可能还在，但工具已经开始做更强的自动适配 | 已进入需要聚焦审查的语义敏感区，通常落在 `L3` |
| `regenerated` | 工具根据目标文件重新生成上下文 | 说明原始 context 已经对不上，需要算法重建 | 已不是低风险文本应用，而是需要确认定位正确的语义适配 |
| `conflict-adapted` | 工具发现真实冲突，并用目标代码重写 `-` 行后再尝试落地 | 这不是“补丁打上了”，而是“冲突被改写后才打上了” | 已经进入高风险审批区，通常至少 `L4` |

一句话解释 `3way`：

- 不是“补丁很安全”
- 而是“Git 帮你把两边改动拼在了一起”
- 真正要确认的是：**拼完之后，逻辑还是不是社区修复本来的意思**
