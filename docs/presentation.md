...
# 四维风险收益评估：L0-L5 解决“多难”，四维模型解决“值不值得”

`L0-L5` 回答的是：**这条补丁应该走哪条处理通道**。  
四维风险收益评估回答的是：**这件事到底值不值得优先做、需要投入多大代价做**。

当前代码里的四个维度来自 `RiskBenefitAnalyzer`，固定是这四项：

| 维度 | 英文字段 | 主要看什么 | 典型输入证据 | 对决策的意义 |
|---|---|---|---|---|
| **合入复杂度** | `merge_complexity` | 这条补丁“合进去”有多难 | DryRun 是否干净通过、冲突文件数、冲突 hunk 严重度、强/中依赖前置补丁数量 | 判断实施成本和落地阻力 |
| **回归风险** | `regression_risk` | 合进去之后“出问题”的概率有多大 | 修改文件数、hunk 数、PatchReview 的 critical/warning、是否涉及锁/RCU/mutex 等并发原语、修改函数集中度 | 判断是否需要更强测试和更高级别审查 |
| **变更范围** | `change_scope` | 这次改动“波及面”有多大 | 改动代码行数、文件数、函数数、是否跨多个子系统 | 判断影响面和验证半径 |
| **安全收益** | `security_benefit` | 这条补丁“修复价值”有多高 | 漏洞类型、严重度/CVSS、影响子系统、利用条件、潜在危害 | 判断这件事是否值得优先投入资源 |
...
因此，真正的维护决策不是只看一个 `final_level`，而是同时看两件事：

1. `L0-L5`：告诉你这条补丁要走多重的处理通道
2. 四维风险收益：告诉你这件事值不值得优先投入、该投入多少验证成本

一个更好讲的管理层口径是：

| 组合 | 应该怎么理解 |
|---|---|
| **低等级 + 高收益** | 优先快速落地，属于“低阻力高回报” |
| **高等级 + 高收益** | 值得做，但必须走专家审查和更强验证 |
| **低等级 + 低收益** | 可以排进常规窗口，不必抢最高优先级 |
| **高等级 + 低收益** | 成本高、收益低，应先补证据或重新评估是否值得立即推进 |

一句话概括：

- `L0-L5` 决定**怎么做**
- 四维风险收益决定**值不值得做、该优先到什么程度**
