...
**设计背景**：在应用社区补丁到自维护内核仓库时，补丁中的 `-` 行（待删除代码）和 context 行（上下文）可能因版本差异而无法在目标文件中直接找到。定位算法需要在"代码行可能已被部分修改、行号已偏移、甚至函数内结构已调整"的情况下，仍能准确找到对应代码区域。

**关键修复**：Diff 解析器 `_parse_hunks_for_regen` 的 `elif` 顺序存在严重 bug — `---`/`+++` 判断在 `current_hunk_header` 判断之前，导致 hunk 内以 `---` 开头的代码行（如 C 语言 `--count` 运算符的删除行 `---count`）被错误吞入 header，使 hunk 内容缺损、序列匹配必然失败。修复后将 hunk 内容捕获移至 `---`/`+++` 判断之前。

**七策略渐进式定位：**

```
策略1: 精确序列匹配
  │   strip 后完全一致, O(N×M) 全文扫描
  │   失败 ↓
策略2: 函数名锚点搜索 [新增]
  │   从 @@ ... @@ func_name() 提取函数名
  │   在目标文件中找到该函数的作用域 (大括号平衡)
  │   在函数作用域内精确/模糊搜索
  │   失败 ↓
策略3: 行号提示 ± 窗口搜索
  │   从 @@ -X,Y @@ 提取 X, 在 [X-200, X+200] 窗口内搜索
  │   先精确, 再逐行模糊 (阈值 0.50)
  │   失败 ↓
策略4: 全局逐行模糊匹配
  │   对每个位置, 逐行 SequenceMatcher 加权评分
  │   短序列 (≤3 行) 阈值降至 0.45, 长序列 0.50
  │   失败 ↓
策略5: Context 行重试
  │   用 hunk 的 context 行 (非 +/-) 重新执行策略1/4
  │   失败 ↓
策略6: 逐行独立投票 [新增]
  │   每行独立在文件中查找最佳匹配位置
  │   收集所有命中位置, 取聚簇中位数作为起始位置
  │   解决"序列整体不匹配但大部分行可独立找到"的场景
  │   失败 ↓
策略7: 最长行最佳匹配
      选序列中最长的行 (信息量最大) 做单行 fuzzy match
```

**逐行模糊评分 (`_line_fuzzy_score`)：**

```python
for a, b in zip(needle_lines, haystack_lines):
    weight = max(1.0, len(a) / 10.0)  # 长行权重高, 避免 { } 等短行干扰
    score = 1.0 if a == b else SequenceMatcher(None, a, b).ratio()
    total += weight * score
return total / total_weight
```

**函数名锚点搜索 (`_find_in_function`)：**

```
@@ -1234,10 +1234,12 @@ static int ip6_tnl_xmit(struct sk_buff *skb)
                                              ↑ 提取函数名 ip6_tnl_xmit

→ 在目标文件中搜索 "ip6_tnl_xmit" → 找到函数起始行
→ 追踪大括号平衡找到函数结束行
→ 在 [func_start, func_end] 范围内精确/模糊搜索
```

函数名通常跨版本保持稳定，即使行号大幅偏移也能定位到正确区域。

**逐行投票算法 (`_find_by_line_voting`)：**

```
needle (补丁 - 行):                    文件中的匹配位置:
  "if (err < 0)"          ──────→     行 1247 (精确匹配)
  "	goto cleanup;"         ──────→     行 1249 (精确匹配)
  "kfree(buf);"            ──────→     行 1250 (精确匹配)
  "mutex_unlock(&lock);"   ──────→     行 1252 (模糊匹配 0.85)

→ 位置聚簇: [1247, 1249, 1250, 1252] → 中位数 1249
→ 估算起始位置: 1249 - needle_offset = 1247 ✔
```

**预处理**：所有搜索序列过滤 `\ No newline at end of file` 等非代码行，避免污染匹配。
