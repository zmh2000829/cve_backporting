...
**五级自适应 DryRun 引擎** — `strict → -C1 → 3way → 上下文重生成 → 冲突适配` 渐进式策略，自动应对从"context 偏移"到"同行代码被修改"的各类冲突场景。**七策略渐进式代码定位算法**（精确匹配 → 函数名锚点 → 行号窗口 → 逐行模糊 → context 重试 → 逐行投票 → 最长行匹配）确保即使跨版本代码结构大幅偏移仍能精准定位。路径映射感知自动翻译跨版本文件路径，优先选用 stable backport 补丁。全部失败时提供**逐 hunk 冲突分析**：精确到行的"补丁期望 vs 文件实际"对比、L1/L2/L3 三级冲突严重度分级、可应用的冲突适配补丁自动生成。
...
DryRun Agent 在重建补丁 context 和分析冲突时，需要在目标文件中定位补丁 hunk 对应的代码区域。这是整个系统中技术难度最高的环节：

```
策略1  精确序列匹配 (strip 后完全一致)
  ↓ 失败
策略2  函数名锚点搜索 (从 @@ 行提取函数名, 限定函数作用域内搜索)
  ↓ 失败
策略3  行号提示窗口搜索 (hunk header @@ -X @@ 提供行号, ±200 行窗口)
  ↓ 失败
策略4  全局逐行模糊匹配 (SequenceMatcher 加权评分, 长行权重高)
  ↓ 失败
策略5  Context 行重试 (用 hunk 的 context 行替代 - 行重新搜索)
  ↓ 失败
策略6  逐行独立投票 (每行独立匹配, 收集位置聚簇, 取中位数)
  ↓ 失败
策略7  最长行最佳匹配 (选信息量最大的行做 fuzzy match)
```

**函数名锚点**利用了一个关键洞察：函数名跨版本高度稳定，即使行号偏移数百行，函数名仍能精确锚定代码区域。**逐行投票**解决了"序列整体因个别行差异无法匹配、但大部分行可独立找到"的场景。
...
2. **七策略渐进式代码定位**：函数名锚点 + 逐行投票等策略组合，在跨版本代码大幅分歧时仍能精确定位
3. **五级自适应补丁应用**：从 strict 到冲突适配的渐进式降级，最大化自动适配能力
4. **闭环验证框架**：基于 git worktree 的非破坏性回退验证，Precision/Recall/F1 量化工具置信度
5. **跨版本路径映射**：双向路径翻译贯穿搜索、比对、DryRun 全链路，彻底解决目录重组盲区
